...
command! -buffer VimwikiSplitWord call vimwiki#WikiSplitWord('split')
command! -buffer VimwikiVSplitWord call vimwiki#WikiVSplitWord('vsplit')
...
function! vimwiki#WikiRenameWord() "{{{
  ...
  " it seems that linux and windows does expand('%') in a different way
  if glob(expand('%:p')) == ''
  ...
  execute ':silent args '.g:vimwiki_home.'*'.g:vimwiki_ext
  if isOldWordComplex
